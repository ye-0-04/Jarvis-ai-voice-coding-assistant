...
class Jarvis:
    def __init__(self):
        ...
        print(f"🧠 Using {AI_PROVIDER}: {model_name}")
    ...
    def _call_ollama(self, prompt):
        ...
        try:
            ...
            print(f"🔄 Trying fallback model {OLLAMA_FALLBACK}...")
            response = ollama.chat(
                model=OLLAMA_FALLBACK, messages=[{"role": "user", "content": prompt}]
            )
            return response["message"]["content"]
    ...
    def _build_prompt(self, question, code_context):
        return f"""You are Jarvis, an AI coding assistant. Follow these rules STRICTLY:
...
YOUR ANSWER (based ONLY on the code above):"""
    ...
    def _validate_answer(self, answer, question, code_context):
        if "couldn't find" not in answer.lower() and "searched" not in answer.lower():
